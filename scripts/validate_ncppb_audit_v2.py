...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--require-reviewed",
        action="store_true",
        help="Require the pair-level BioSample review queue to be empty",
    )
    parser.add_argument(
...
def main() -> None:
    ...
    manual_candidates = read_table(args.outdir / "manual_review_candidates.tsv")
    ...
    explorer = read_table(args.outdir / "explorer_strain_catalogue.tsv")
    ...
    if len(explorer) != len(snapshot_diff):
        errors.append(f"explorer rows={len(explorer)}, snapshot diff rows={len(snapshot_diff)}")
    ...
    missing_explorer = [row for row in explorer if row.get("snapshot_status") == "missing_from_v2_snapshot"]
    if any(
        any(int(row.get(column, "0") or 0) for column in [
            "ncbi_biosample_match_count", "ncbi_assembly_count", "ncbi_sra_run_count",
            "ncbi_bioproject_count", "ncbi_sequence_resource_count", "ncbi_record_match_count",
            "ncbi_eligible_assembly_count", "ncbi_eligible_sra_run_count",
            "ncbi_eligible_sequence_resource_count", "ncbi_taxonomy_blocked_resource_count",
        ])
        for row in missing_explorer
    ):
        errors.append("historical rows missing from the current snapshot must have zero current NCBI counts")
    ...
    invalid_selected_resources = [
        ...
        and (
            ...
            or row.get("downstream_eligible") != "yes"
            or (
                row.get("taxonomy_status") not in {"same_name", "compatible_synonym"}
                or row.get("pathovar_status") == "pathovar_mismatch"
            )
            and row.get("reviewer_approved_for_downstream") != "yes"
        )
    ]
    ...
    blocked_selected = [
        row for row in resources
        if row.get("selected_for_phylogeny") == "yes" and row.get("downstream_block_reason")
    ]
    if blocked_selected:
        errors.append(f"taxonomy/identity-blocked resources selected for phylogeny: {len(blocked_selected)}")
    manifest_selected = {
        (row.get("ncppb_number", ""), accession)
        for row in phylogeny
        for accession in row.get("preferred_sequence_accessions", "").split("; ")
        if accession
    }
    resource_selected = {
        (row.get("ncppb_number", ""), row.get("resource_accession", ""))
        for row in resources
        if row.get("selected_for_phylogeny") == "yes"
    }
    if manifest_selected != resource_selected:
        errors.append(
            f"phylogeny/resource selection mismatch: manifest={len(manifest_selected)}, resources={len(resource_selected)}"
        )
    ...
    candidate_queue_numbers = {row["ncppb_number"] for row in manual_candidates}
    if candidate_queue_numbers != queue_numbers:
        errors.append(
            f"pair-level manual queue mismatch: strain_queue={len(queue_numbers)}, pair_queue={len(candidate_queue_numbers)}"
        )
    if args.require_reviewed and manual_candidates:
        errors.append(f"unresolved pair-level BioSample reviews: {len(manual_candidates)}")
    required_cache_parameters = {"refresh_cache", "cache_max_age_hours", "offline_cache_only"}
    missing_cache_parameters = required_cache_parameters - set(manifest.get("parameters", {}))
    if missing_cache_parameters:
        errors.append(f"run manifest missing cache policy fields: {sorted(missing_cache_parameters)}")
    hash_mismatches: list[str] = []
    for relative, metadata in manifest.get("outputs", {}).items():
        path = args.outdir / relative
    ...
    if hash_mismatches:
        errors.append(f"run manifest output hash/size mismatches: {len(hash_mismatches)}")
    ...
    print(f"INFO: explorer_rows={len(explorer)}")
    ...
    print(f"INFO: manual_review_candidates={len(manual_candidates)}")
    print(f"INFO: taxonomy_blocked_resources={sum(row.get('downstream_block_reason') == 'taxonomy_review_required' for row in resources)}")
    ...
    if errors:
        for error in errors:
            print(f"ERROR: {error}")
        raise SystemExit(1)
    print("V2.1.1 audit validation passed.")
