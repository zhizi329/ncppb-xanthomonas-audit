...
class NcbiClient:
    def __init__(
        ...
        if offline_cache_only and refresh_cache:
            raise ValueError("refresh_cache cannot be combined with offline_cache_only")
        if cache_max_age_hours < 0:
            raise ValueError("cache_max_age_hours must be zero or positive")
        self.refresh_cache = refresh_cache
        self.cache_max_age_hours = cache_max_age_hours
        self.request_count = 0
        self.cache_hits = 0
        self.stale_cache_misses = 0
    ...
    def request(self, endpoint: str, params: dict[str, str]) -> tuple[str, Path]:
        ...
        cache_expired = False
        if cache_path.exists() and self.cache_max_age_hours:
            age_hours = max(0.0, (time.time() - cache_path.stat().st_mtime) / 3600.0)
            cache_expired = age_hours > self.cache_max_age_hours
        use_cache = cache_path.exists() and not self.refresh_cache and not cache_expired
        if use_cache:
        ...
        if cache_expired:
            with self._counter_lock:
                self.stale_cache_misses += 1
        if self.offline_cache_only:
            reason = "expired" if cache_expired else "missing_or_invalid"
            raise RuntimeError(f"NCBI cache {reason}: {endpoint} {params}")
        url = (
...
def parse_biosamples(xml_text: str, cache_path: Path) -> dict[str, dict[str, str]]:
    ...
    for sample in root.findall(".//BioSample"):
        ...
        for attribute in sample.findall("./Attributes/Attribute"):
            ...
            context_mapped = CONTEXT_ATTRIBUTE_NAMES.get(normalized_name)
            if context_mapped:
                context_values.setdefault(context_mapped, []).append(value)
        ids: list[str] = []
        ...
        records[uid or accession] = {
            ...
            "host": unique_join(context_values.get("host", [])),
            "geographic_location": unique_join(context_values.get("geographic_location", [])),
            "collection_date": unique_join(context_values.get("collection_date", [])),
            "isolation_source": unique_join(context_values.get("isolation_source", [])),
                ...
                task_rows.append(
                    {
                        ...
                        "link_method": "ncbi_biosample_elink",
                        "verification_status": "verified_by_ncbi_elink",
                        "provenance": "NCBI ELink and ESummary",
            ...
            task_rows.append(
                {
                    ...
                    "linked_database": database,
                    "link_method": "ncbi_biosample_elink",
                    "verification_status": "link_error",
                    "status": "error",
                    "error": f"{type(exc).__name__}: {exc}",
                }
            )
...
def merge_verified_resource_seeds(
    linked_rows: list[dict[str, str]],
    seed_rows: list[dict[str, str]],
    match_rows: list[dict[str, str]],
) -> list[dict[str, str]]:
    """Add externally discovered accessions only after an explicit BioSample-pair verification.

    This is the controlled fallback for resources missed by BioSample ELink (for
    example an accession found in PhytoBacExplorer). A seed must state
    ``verification_status=verified_against_biosample`` and refer to a currently
    accepted strain--BioSample pair. It remains visibly marked as an external seed.
    """
    accepted = {
        (row.get("ncppb_number", ""), row.get("biosample_accession", "").upper())
    ...
    patterns = {
        ...
        "sra": re.compile(r"^[SED]RR\d+$", re.I),
        "bioproject": re.compile(r"^PRJ[A-Z]{2}\d+$", re.I),
    }
    output = [dict(row) for row in linked_rows]
    existing = {
        (
            row.get("ncppb_number", ""),
            row.get("biosample_accession", "").upper(),
            row.get("linked_database", ""),
            row.get("linked_accession", "").upper(),
        )
        for row in output
        if row.get("status") == "ok"
    }
    for seed in seed_rows:
        number = clean_text(seed.get("ncppb_number", ""))
        biosample = clean_text(seed.get("biosample_accession", "")).upper()
        database = clean_text(seed.get("linked_database", seed.get("resource_type", ""))).lower()
        database = "sra" if database in {"sra_run", "sra"} else database
        accession = clean_text(seed.get("linked_accession", seed.get("resource_accession", ""))).upper()
        verification = clean_text(seed.get("verification_status", "")).lower()
        if verification != "verified_against_biosample":
            raise ValueError(
                f"resource seed requires verification_status=verified_against_biosample: {number} {accession}"
            )
        if (number, biosample) not in accepted:
            raise ValueError(f"resource seed is not attached to an accepted BioSample pair: {number} {biosample}")
        if database not in patterns or not patterns[database].fullmatch(accession):
            raise ValueError(f"invalid {database or 'resource'} seed accession: {accession}")
        key = (number, biosample, database, accession)
        if key in existing:
            continue
        existing.add(key)
        source_url = clean_text(seed.get("source_url", "")) or f"https://www.ncbi.nlm.nih.gov/{database}/{accession}"
        output.append(
            {
                "ncppb_number": number,
                "biosample_uid": clean_text(seed.get("biosample_uid", "")),
                "biosample_accession": biosample,
                "linked_database": database,
                "linked_uid": clean_text(seed.get("linked_uid", "")),
                "linked_accession": accession,
                "title": clean_text(seed.get("title", "")),
                "assembly_level": clean_text(seed.get("assembly_level", "")),
                "link_method": "verified_external_seed",
                "verification_status": "verified_against_biosample",
                "provenance": clean_text(seed.get("provenance", seed.get("source", "external_resource_seed"))),
                "extra_json": clean_text(seed.get("extra_json", "")) or "{}",
