...
def build_identifier_review_queue(identifiers: list[dict[str, str]]) -> list[dict[str, str]]:
    """Expose every potentially actionable disabled identifier, not preserved prose."""
    rows = [
        {**row, "reviewer_decision": "", "reviewer_notes": ""}
        for row in identifiers
        if row.get("validation_status") == "review_required"
        and row.get("identifier_type") != "source_reference_raw"
    ]
    return sorted(
        rows,
        key=lambda row: (
            int(re.search(r"\d+", row.get("ncppb_number", "0")).group(0)),
            row.get("identifier_raw", ""),
        ),
    )


def write_identifier_outputs(outdir, identifiers, reviews) -> list[dict[str, str]]:
    identifier_reviews = build_identifier_review_queue(identifiers)
    ...
    write_table(outdir / "identifier_review_queue.tsv", identifier_reviews, IDENTIFIER_REVIEW_COLUMNS)
    return identifier_reviews
