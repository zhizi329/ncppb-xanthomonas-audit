...
MATCH_COLUMNS = [
    ...
    "taxid",
    "biosample_host",
    "biosample_geographic_location",
    "biosample_collection_date",
    "biosample_collection_year",
    ...
    "reviewer_decision",
    "reviewer_notes",
    "reviewer_approved_for_downstream",
    "review_reason",
...
SUPERVISOR_COLUMNS = [
    ...
    "catalogue_country",
    "biosample_hosts",
    "biosample_geographic_locations",
    "biosample_collection_dates",
    "biosample_collection_years",
    "biosample_isolation_sources",
    "metadata_annotation_status",
    "pathogenicity_annotation_status",
    "snapshot_status",
    "all_searchable_identifiers",
    "confirmed_biosample_accessions",
    "provisional_biosample_accessions",
    "downstream_eligible_biosample_accessions",
...
def pathovar(value: str) -> str:
    match = re.search(r"\b(?:pv\.?|pathovar)\s+([A-Za-z0-9_-]+)", value or "", re.IGNORECASE)
    return match.group(1).lower() if match else ""


def pathovar_status(strain: dict[str, str], candidate: dict[str, str]) -> str:
    expected_values = [
        strain.get("current_name_raw", ""),
        strain.get("name_as_received", ""),
        strain.get("alternative_names", ""),
    ]
    expected = next((pathovar(value) for value in expected_values if pathovar(value)), "")
    observed = pathovar(candidate.get("organism", ""))
    if not expected:
        return "not_applicable_no_expected_pathovar"
    if not observed:
        return "observed_pathovar_unresolved"
    return "same_pathovar" if observed == expected else "pathovar_mismatch"


CONTEXT_ATTRIBUTE_ALIASES = {
    "host": {"host", "host_scientific_name", "host_common_name"},
    "geographic_location": {"geo_loc_name", "geographic_location", "country"},
    "collection_date": {"collection_date"},
    "isolation_source": {"isolation_source"},
}


def candidate_context_value(candidate: dict[str, str], target: str) -> str:
    direct = clean_text(candidate.get(target, ""))
    if direct:
        return direct
    try:
        attributes = json.loads(candidate.get("attributes_json", "") or "{}")
    except json.JSONDecodeError:
        return ""
    values: list[str] = []
    aliases = CONTEXT_ATTRIBUTE_ALIASES[target]
    for key, raw_values in attributes.items() if isinstance(attributes, dict) else []:
...
def collection_year(value: str) -> str:
    return unique_join(re.findall(r"(?<!\d)(?:18|19|20)\d{2}(?!\d)", value or ""))
...
SAFE_TAXONOMY_STATUSES = {"same_name", "compatible_synonym"}
APPROVE_DECISIONS = {"approve_for_downstream", "approve"}
REJECT_DECISIONS = {"reject_match", "reject"}


def downstream_identity_eligible(row: dict[str, str]) -> bool:
    """Separate identity retention from permission to use sequence data downstream."""
    automatically_safe = (
        row.get("taxonomy_status") in SAFE_TAXONOMY_STATUSES
        and row.get("pathovar_status") != "pathovar_mismatch"
    )
    return row.get("decision") == "accept" and (
        automatically_safe or row.get("reviewer_approved_for_downstream") == "yes"
    )


def apply_review_decisions(
    match_rows: list[dict[str, str]], review_rows: list[dict[str, str]]
) -> list[dict[str, str]]:
    """Apply auditable pair-level review decisions without losing the automatic result.

    The input key is (ncppb_number, biosample_accession). Supported decisions are
    ``approve_for_downstream``, ``reject_match`` and ``keep_pending``.
    """
    indexed: dict[tuple[str, str], dict[str, str]] = {}
    valid = APPROVE_DECISIONS | REJECT_DECISIONS | {"keep_pending", "pending", ""}
    for review in review_rows:
        key = (clean_text(review.get("ncppb_number", "")), clean_text(review.get("biosample_accession", "")).upper())
        decision = clean_text(review.get("reviewer_decision", "")).lower()
        if not all(key):
            raise ValueError("review decision rows require ncppb_number and biosample_accession")
        if decision not in valid:
            raise ValueError(f"unsupported reviewer_decision={decision!r} for {key[0]} {key[1]}")
        if key in indexed and clean_text(indexed[key].get("reviewer_decision", "")).lower() != decision:
            raise ValueError(f"conflicting review decisions for {key[0]} {key[1]}")
        indexed[key] = review

    output: list[dict[str, str]] = []
    matched_keys: set[tuple[str, str]] = set()
    for source in match_rows:
        row = dict(source)
        row.setdefault("original_decision", row.get("decision", ""))
        row.setdefault("reviewer_decision", "")
        row.setdefault("reviewer_notes", "")
        row.setdefault("reviewer_approved_for_downstream", "no")
        key = (row.get("ncppb_number", ""), row.get("biosample_accession", "").upper())
        review = indexed.get(key)
        if review:
            matched_keys.add(key)
            decision = clean_text(review.get("reviewer_decision", "")).lower()
            row["reviewer_decision"] = decision
            row["reviewer_notes"] = clean_text(review.get("reviewer_notes", ""))
            if decision in APPROVE_DECISIONS:
                if row.get("original_decision") not in {"accept", "review"}:
                    raise ValueError(
                        f"cannot approve an automatically rejected candidate: {key[0]} {key[1]}"
                    )
                row["decision"] = "accept"
                row["identity_review_required"] = "no"
                row["taxonomy_review_required"] = "no"
                row["manual_review_required"] = "no"
                row["reviewer_approved_for_downstream"] = "yes"
                row["review_reason"] = "reviewer_approved_for_downstream"
            elif decision in REJECT_DECISIONS:
                row["decision"] = "reject"
                row["identity_review_required"] = "no"
                row["taxonomy_review_required"] = "no"
                row["manual_review_required"] = "no"
                row["reviewer_approved_for_downstream"] = "no"
                row["review_reason"] = "reviewer_rejected_match"
        output.append(row)
    unknown = set(indexed) - matched_keys
    if unknown:
        example = sorted(unknown)[0]
        raise ValueError(f"review decision does not match a candidate: {example[0]} {example[1]}")
    return output
...
def classify_candidate(
    ...
    if candidate.get("status") == "error":
        return {
            ...
            "pathovar_status": "observed_pathovar_unresolved",
    ...
    for identifier in searchable:
        ...
        pv_status = pathovar_status(strain, candidate)
        strength = effective_identifier_strength(identifier)
        strong_identifier = strength in {"primary", "strong"}
        taxonomy_review = tax_status not in SAFE_TAXONOMY_STATUSES or pv_status == "pathovar_mismatch"
        if conflicts:
            decision = "review"
            reason = "conflicting_ncppb_identifier"
        elif strong_identifier:
            decision = "accept"
            reason = "pathovar_mismatch" if pv_status == "pathovar_mismatch" else tax_status if taxonomy_review else ""
        else:
            decision = "review"
            reason = "medium_identifier_requires_corroboration"
            if taxonomy_review:
                reason += f"; {'pathovar_mismatch' if pv_status == 'pathovar_mismatch' else tax_status}"
        identity_status = (
    ...
    for identifier in searchable:
        ...
        if title_match(raw, candidate):
            ...
            return {
                ...
                "pathovar_status": pathovar_status(strain, candidate),
                "identity_review_required": "yes",
        ...
        rows.append(
            {
                ...
                "biosample_host": candidate_context_value(representative, "host"),
                "biosample_geographic_location": candidate_context_value(representative, "geographic_location"),
                "biosample_collection_date": candidate_context_value(representative, "collection_date"),
                "biosample_collection_year": collection_year(
                    candidate_context_value(representative, "collection_date")
                ),
                "biosample_isolation_source": candidate_context_value(representative, "isolation_source"),
                **evidence,
                "original_decision": evidence.get("decision", ""),
                "reviewer_decision": "",
                "reviewer_notes": "",
                "reviewer_approved_for_downstream": "no",
...
def build_supervisor_table(
    ...
    for strain in strains:
        ...
        eligible_accessions = {
            row.get("biosample_accession", "") for row in accepted if downstream_identity_eligible(row)
        }
        blocked_accessions = accepted_set - eligible_accessions
        provisional_set = set(provisional_accessions)
        links = [
            row for row in links_by_strain.get(number, [])
            if row.get("biosample_accession", "") in eligible_accessions
        ]
        provisional_links = [
        ...
        if blocked_accessions and not links:
            category = "taxonomy_review_required_before_sequence_use"
        ...
        metadata_fields = {
            "host": unique_join(row.get("biosample_host", "") for row in accepted),
            "geography": unique_join(row.get("biosample_geographic_location", "") for row in accepted),
            "date": unique_join(row.get("biosample_collection_date", "") for row in accepted),
            "year": unique_join(row.get("biosample_collection_year", "") for row in accepted),
            "source": unique_join(row.get("biosample_isolation_source", "") for row in accepted),
        }
        available_metadata = sum(bool(value) for key, value in metadata_fields.items() if key != "year")
        metadata_status = (
            "complete_host_geography_date_source"
            if available_metadata == 4
            else "partial_context_metadata"
            if available_metadata
            else "missing_context_metadata"
        )
        output.append(
            {
                "ncppb_number": number,
                ...
                "scope_status": (
                    "current_xanthomonas"
                    if strain.get("expected_genus", "").lower() == "xanthomonas"
                    else "reclassified_outside_xanthomonas"
                ),
                "catalogue_host": strain.get("host", ""),
                "catalogue_country": strain.get("country", ""),
                "biosample_hosts": metadata_fields["host"],
                "biosample_geographic_locations": metadata_fields["geography"],
                "biosample_collection_dates": metadata_fields["date"],
                "biosample_collection_years": metadata_fields["year"],
                "biosample_isolation_sources": metadata_fields["source"],
                "metadata_annotation_status": metadata_status,
                "pathogenicity_annotation_status": "not_assessed_requires_external_or_phenotype_data",
                ...
                "downstream_eligible_biosample_accessions": unique_join(sorted(eligible_accessions)),
                "taxonomy_blocked_biosample_accessions": unique_join(sorted(blocked_accessions)),
