...
def write_summary(
    ...
    lines = [
        ...
        f"- Actionable identifier review rows: {len(actionable_identifier_reviews)}",
    ...
    if match_rows is not None and supervisor_rows is not None:
        ...
        lines.extend(
            [
                ...
                f"- Scope status: {count_values(supervisor_rows, 'scope_status')}",
                f"- Pathovar comparison: {count_values(match_rows, 'pathovar_status')}",
