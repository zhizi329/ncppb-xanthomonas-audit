...
from .matching import downstream_identity_eligible
...
def build_retrieval_manifests(
    ...
    def base_row(number: str, accession: str) -> dict[str, str]:
        ...
        eligible = evidence_status == "confirmed" and downstream_identity_eligible(match)
        if evidence_status != "confirmed":
            block_reason = "identity_not_confirmed"
        elif match.get("decision") != "accept":
            block_reason = "match_not_accepted"
        elif not eligible:
            block_reason = "taxonomy_review_required"
        else:
            block_reason = ""
        return {
            "ncppb_number": number,
            ...
            "pathovar_status": match.get("pathovar_status", ""),
            "reviewer_approved_for_downstream": match.get("reviewer_approved_for_downstream", "no"),
            "downstream_eligible": "yes" if eligible else "no",
                        ...
                        resource_rows.append(
                            {
                                **base,
                                ...
                                "sequence_scope": "whole_genome_assembly",
                                "assembly_level": linked.get("assembly_level", ""),
                                "assembly_source": metadata.get("assembly_source", "") or (
                                    "RefSeq" if accession.upper().startswith("GCF_") else "GenBank"
                                ),
                                ...
                                "link_method": linked.get("link_method", ""),
                                "link_verification_status": linked.get("verification_status", ""),
                                "link_provenance": linked.get("provenance", ""),
                        ...
                        resource_rows.append(
                            {
                                **base,
                                ...
                                "sequence_scope": (
                                    "whole_genome_shotgun_reads"
                                    if str(metadata.get("library_strategy", "")) == "WGS"
                                    else "non_wgs_or_unresolved_reads"
                                ),
        ...
        resource_rows.append(
            {
                **base,
                ...
                "resource_role": role,
                "sequence_scope": "project_provenance_only",
                "download_tool": "NCBI BioProject metadata",
                "link_method": "derived_from_sequence_metadata_or_elink",
                "source_url": url,
            }
        )
    ...
    for supervisor in supervisor_rows:
        ...
        assemblies = [
            ...
            if row.get("downstream_eligible") == "yes" and row.get("resource_type") == "assembly"
        ]
        wgs_runs = [
            row
            for row in resources
            if row.get("downstream_eligible") == "yes"
            and row.get("resource_type") == "sra_run"
            and row.get("sra_library_strategy") == "WGS"
            and row.get("sra_library_source") in {"", "GENOMIC"}
        ]
        selected: list[dict[str, str]] = []
        blocked_sequence_rows = [
            row for row in resources
            if row.get("evidence_status") == "confirmed"
            and row.get("downstream_eligible") != "yes"
            and row.get("resource_type") in {"assembly", "sra_run"}
        ]
        preferred_type = "none"
        ...
        if assemblies:
            ...
            if blocked_sequence_rows:
                readiness = "taxonomy_review_required_before_use"
                reason = "Linked sequence exists, but taxonomy review blocks automatic downstream selection."
            else:
        ...
        phylogeny_rows.append(
            {
                ...
                "preferred_sequence_scope": unique_join(row.get("sequence_scope", "") for row in selected),
                ...
                "selection_block_reason": unique_join(
                    row.get("downstream_block_reason", "") for row in blocked_sequence_rows
                ),
