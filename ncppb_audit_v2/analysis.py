...
MANUAL_REVIEW_CANDIDATE_COLUMNS = [
    "review_priority",
    "ncppb_number",
    "ncppb_current_name",
    "biosample_accession",
...
def review_question(rows: list[dict[str, str]]) -> str:
    ...
    if any("pathovar_mismatch" in reason for reason in reasons):
        return "Is the pathovar difference a documented relabeling, or does it indicate the wrong strain record?"
    if any("same_genus_different_species" in reason for reason in reasons):
        ...
        output.append(
            {
                ...
                "pathovar_statuses": unique_join(row.get("pathovar_status", "") for row in rows),
                ...
                "reviewer_decision": unique_join(row.get("reviewer_decision", "") for row in rows),
                "reviewer_notes": unique_join(row.get("reviewer_notes", "") for row in rows),
            }
        )
...
def build_manual_review_candidates(
    ...
    """Write one reviewable row per strain--BioSample pair for round-trip decisions."""
    names = {row["ncppb_number"]: row.get("ncppb_current_name", "") for row in supervisor_rows}
    output: list[dict[str, str]] = []
    for row in match_rows:
        if row.get("manual_review_required") != "yes":
            continue
        ...
        output.append(
            {
                ...
                "ncppb_current_name": names.get(row.get("ncppb_number", ""), ""),
                "biosample_accession": row.get("biosample_accession", ""),
                "organism": row.get("organism", ""),
                "automatic_decision": row.get("original_decision", row.get("decision", "")),
                "matched_identifier": row.get("matched_identifier", ""),
                "matched_identifier_type": row.get("matched_identifier_type", ""),
                "matched_field": row.get("matched_field", ""),
                "evidence_class": row.get("evidence_class", ""),
                "taxonomy_status": row.get("taxonomy_status", ""),
                "pathovar_status": row.get("pathovar_status", ""),
                "review_reason": row.get("review_reason", ""),
                "biosample_url": row.get("source_url", ""),
                "review_question": review_question([row]),
...
def write_analysis_outputs(
    ...
    manual_candidate_rows: list[dict[str, str]] | None = None,
) -> None:
    write_table(outdir / "manual_review_queue.tsv", manual_rows, MANUAL_REVIEW_COLUMNS)
    write_table(
        outdir / "manual_review_candidates.tsv",
        manual_candidate_rows or [],
        MANUAL_REVIEW_CANDIDATE_COLUMNS,
    )
